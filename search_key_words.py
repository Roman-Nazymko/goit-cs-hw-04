...
def search_keywords_in_file(file_path, keywords, result_dict, identifier):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            print(f"{Fore.GREEN}{identifier}: File '{file_path}' read.{Style.RESET_ALL}")
            content = file.read()
        ...
        print(f"{Fore.RED}{identifier}: Error reading file '{file_path}': {e}{Style.RESET_ALL}")
...
def main():
    ...
    print(f"{Fore.YELLOW}Time taken: {threading_time:.2f} seconds{Style.RESET_ALL}\n")
    ...
    for keyword, files in multiprocessing_results.items():
        print(f"{Fore.LIGHTCYAN_EX}{keyword}: {files}")
    print(f"{Fore.YELLOW}Time taken: {multiprocessing_time:.2f} seconds{Style.RESET_ALL}\n")
